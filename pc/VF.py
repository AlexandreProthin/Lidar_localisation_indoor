...
def estUnMur(cluster, murs):
    ...
    """
    for mur in murs:
        if mur[0]-5 < cluster[0] < mur[0] + 5 and mur[1]-100 < cluster[1] < mur[1] + 100:
            return True
    return False
...
# n pour le facteur d'échelle
n = 10
def redrawGameWindow(personnes, xLidar, yLidar):
    # win.blit(bg, (0,0)) # commenter cette ligne pour visualiser la trace
    for personne in personnes:
        x, y = PolarToCart(personne[0], personne[1]/n)
        print([x,y])
        win.blit(pers, (x+xLidar, y+yLidar))
    pygame.display.update()

def redrawGameWindow1(personnes, xLidar, yLidar):
    # win.blit(bg, (0,0)) # commenter cette ligne pour visualiser la trace
    for personne in personnes:
        x, y = PolarToCart(personne[0], personne[1]/n)
        win.blit(mr, (x+xLidar, y+yLidar))
    pygame.display.update()


def redrawGameWindow3(personnes, xLidar, yLidar):
    noir = pygame.image.load('noir.png')
    noir = pygame.transform.scale(noir, (3, 3))
    # win.blit(bg, (0,0)) # commenter cette ligne pour visualiser la trace
    for personne in personnes:
        x, y = PolarToCart(personne[0], personne[1])
        win.blit(noir, (x+xLidar, y+yLidar))
    pygame.display.update()
...
while run:
    ...
    # affiche les murs
    # redrawGameWindow1(posMurs, lidarPositionX, lidarPositionY) 
